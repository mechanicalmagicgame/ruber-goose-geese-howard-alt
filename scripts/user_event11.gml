...
for (i = 0; i < array_length_1d(arr[cursor].objs); i++){
	...
	if cur.gimmick == 5 || cur.gimmick == -1 with player_id shader_start();
	...
	if (cur.type == 0){ // Text
		...
		switch(cur.gimmick){
			...
			case 3: // scrolling to the right
				offset_x -= ease_linear(side_bar.screen_w, 0, side_bar.state_timer % 120, 120);
				break;
			case 4: case -1: // don't increase scroll
				break;
		}
	...
	if (cur.gimmick != 4 && cur.gimmick != -1) || i == array_length_1d(arr[cursor].objs) - 1 draw_y += last_drawn[1] + para_margin;
	
	if cur.gimmick == 5 || cur.gimmick == -1 with player_id shader_end();
}
